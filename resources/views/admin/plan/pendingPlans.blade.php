      ...
      <div class="row wrapper border-bottom page-heading">
        <div class="col-lg-12">
          ...
          <ol class="breadcrumb">
            ...
            <li> <a>Plans</a> </li>
            <li class="active"> <strong> Pending Plans </strong> </li>
          </ol>
        </div>
      </div>
      <div class="row">
                ...
                <div >
                  <table id="example7" class="display nowrap table  responsive nowrap table-bordered">
                    ...
                    <tbody>
                      ...
                      <tr>
                        <td>{{$plan->id}}</td>
                        {{-- <td><img class="image_fade" src="{{ asset('/storage/region_images/'. $pendingProfiles->region_image)}}" alt="Standard Post with Image" height="50px;" width="50px;"></td> --}}
                        <td>{{$plan->plan_name}}</td>
                        <td>{{$plan->architect()->first()->name}}</td>
                        <td><a href ="/admin/view-plan/{{$plan->id}}" target="_blank">View</a></td>
                        <td>@if($plan->plan_status == 0)
                            <div class="btn-group">
                                <button class="btn btn-warning" type="button">Pending</button>
                                <button data-toggle="dropdown" class="btn btn-warning dropdown-toggle" type="button"> <i class="fa fa-angle-down"></i> </button>
                                <ul role="menu" class="dropdown-menu">
                                  <li>
                                      <form action="/admin/approve-plan/{{$plan->id}}" method="POST" enctype="multipart/form-data">
                                  ...
                                  <li> <form action="/admin/reject-plan/{{$plan->id}}" method="POST" enctype="multipart/form-data">
                                    <input type="hidden" name="_method" value="PUT">
                                    <input type="hidden" name="id" value="{{$plan->id}}">
                                  @csrf
                                <button type="submit" style="width: 100%" class="btn btn-danger">Reject</button>
                                  </form> </li>
                            ...
                            <div class="btn-group">
                                ...
                                <button data-toggle="dropdown" class="btn btn-success dropdown-toggle" type="button"> <i class="fa fa-angle-down"></i> </button>
                                <ul role="menu" class="dropdown-menu">
                                  <li>
                                      <form action="/admin/approve-plan/{{$plan->id}}" method="POST" enctype="multipart/form-data">
                                        <input type="hidden" name="_method" value="PUT">
                            ...
                            @elseif($plan->plan_status == 2)
                            <div class="btn-group">
                                <button class="btn btn-danger" type="button">Rejected</button>
                                <button data-toggle="dropdown" class="btn btn-danger dropdown-toggle" type="button"> <i class="fa fa-angle-down"></i> </button>
                                <ul role="menu" class="dropdown-menu">
                                  <li>
                                      <form action="/admin/approve-plan/{{$plan->id}}" method="POST" enctype="multipart/form-data">
                                        <input type="hidden" name="_method" value="PUT">
