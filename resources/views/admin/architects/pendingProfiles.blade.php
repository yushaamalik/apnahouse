                  ...
                  <table id="example7" class="display nowrap table  responsive nowrap table-bordered">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{$pendingProfile->company_name}}</td>
                        <td>{{$pendingProfile->website}}</td>
                        <td><a href ="/admin/view-architectProfile/{{$pendingProfile->architect_id}}" target="_blank">View</a></td>
                        <td><form action="/admin/approve-profile/{{$pendingProfile->id}}" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="_method" value="PUT">
                            <input type="hidden" name="id" value="{{$pendingProfile->id}}">
                          @csrf
                        <button type="submit" style="width: 100%" class="btn btn-success">Approve</button>
                        </form>
                        <form action="/admin/reject-profile/{{$pendingProfile->id}}" method="POST" enctype="multipart/form-data">
                          <input type="hidden" name="_method" value="PUT">
                          <input type="hidden" name="id" value="{{$pendingProfile->id}}">
                        @csrf
